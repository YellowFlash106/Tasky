...
const Layout = ({onLogout, user}) => {
  ...
  if(error) return (
    <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
      ...
      <button onClick={fetchTasks} className='mt-4 bg-red-100  text-red-700 font-medium hover:bg-red-200 transition-colors text-sm px-4 py-2 rounded-lg'>
        Try again
      </button>
      </div>
    </div>
        ...
        <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
          <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
            ...
            <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
              ...
              icon={<Circle className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500"/>} />
              <StatCard title="Competed" value={stats.completedTasks} 
              icon={<Circle className="w-4 h-4 sm:w-5 sm:h-5 text-green-500"/>} />
              <StatCard title="Pending" value={stats.pendingCount} 
              icon={<Circle className="w-4 h-4 sm:w-5 sm:h-5 text-fuchsia-500"/>} />
              <StatCard title="Competion Rate" value={`${stats.completionPercentage}% `}
              icon={<Zap className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500"/>} />
            </div>
            ...
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
             ...
             <div className='space-y-2 sm:space-y-3'>
             {tasks.slice(0,3).map((task) =>( <div 
             key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100' >
              ...
              <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2
                ${task.completed ? 'bg-green-100 text-gray-700 ' : 'bg-fuchsia-100  text-fuchsia-700'} `}>
