      ...
      {menuItems.map(({text, path, icon}) => (
        <li key={text}>
          <NavLink to={path} className={({isActive}) =>[
            LINK_CLASSES.base, isActive ? LINK_CLASSES.active : LINK_CLASSES.inactive,
       ...
       <div className={SIDEBAR_CLASSES.mobileDrawerBackdrop} onClick={() => setMobileOpen(false)} />
        <div className={SIDEBAR_CLASSES.mobileDrawer} onClick={(e) => e.stopPropagation()}>
          <div className='flex items-center justify-between mb-4 border-b pb-2 '>
           <h2 className='txt-lg font-bold text-purple-600'>Menu</h2>
        ...
        <div className='flex items-center gap-3 mb-6'>
         <div className='size-10 rounded-full bg-gradient-to-br from-fuchsia-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-md'>
          ...
          <div>
           <h2 className='mt-16 text-lg font-bold text-gray-800'> Hey,   {username} 🐞</h2>
           <p className='text-sm text-purple-500 font-medium flex items-center gap-2'>
           <Sparkles className='size-4' /> Let's crush some  tasks!😎
           </p>
          </div>
