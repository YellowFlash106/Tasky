...
const Signup = ({onSwitchMode}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({text:"Signup successfull! You can now log in", type:"success"});
      setFormData(INITIAL_FORM);
      
    } catch (error) {
      console.error("Signup error",error);
      setMessage({text: error.response?.data?.message || "An error occured. Please try afgain", type: "error"})
    }
    finally{
      setLoading(false);
    }
  ...
  return (
    ...
    <div className='max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8'>
     ...
     <p className='text-gray-500 text-sm mt-1'>
      Join Tasky to manage your tasks efficiently
     </p>
     ...
     <form onSubmit={handleSubmit} className='space-y-4'> 
      ...
      <button type='submit' className={BUTTONCLASSES} disabled={loading} >{loading ? "Signing up..." : <><UserPlus className='w-4 h-4'/>Sign Up</>}
      </button>
     </form>
