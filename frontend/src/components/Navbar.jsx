...
const Navbar = ({user = {}, onLogout}) => {
  ...
  return (
    ...
    backdrop-blur-md shadow-sm border-b border-gray-200 font-sans '>
                ...
                <div ref={menuref} className='relative'>
                    <button onClick={handleMenuToggle} className='flex items-center gap-2 px-3   py-2  rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300 border border-transparent hover:border-purple-200'> 
                        <div className='relative'>
                           { user.avatar ? (<img src={user.avatar} alt='Avatar' className='w-8 h-8 rounded-full shadow-sm' />
                           ) : (
                        ...
                        <div className='absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 animate-pulse border-2 rounded-full '></div>
                        </div>

                        <div className='text-left hidden md:block'>
                            <p className='text-sm font-medium text-gray-800 '>{user.name}</p>
                            <p className='text-xm font-normal text-gray-500 '>{user.email}</p>
                        </div>
                    ...
                    <button onClick={() => {
                        setMenuOpen(false);
                        navigate('/profile');
                    }} className=' w-full px-4 py-2.5 text-left hover:bg-purple-50  text-sm text-gray-700 hover:text-purple-500 transition-colors flex items-center gap-2 group' role="menuitem">
                    <Settings className='w-4 h-4 to-gray-700' />
                    Profile Setting
                    </button>
                      </li>
